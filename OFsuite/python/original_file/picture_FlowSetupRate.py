...
bar=ax.bar(inx+0.2,Y,width,alpha=0.5,color='m',label='Flow_mod rate')#,edgecolor='w',label='packet_out rate')
for i in bar:
#    h =float(i.get_height)
    h = i.get_height()
#    h = i.get_y() 
#   print h,i.get_x(),i.get_width()
    ax.text(i.get_x()+i.get_width()/2.0,h*1.05,'%d'%(h),ha='center',va='bottom',fontsize=10)
