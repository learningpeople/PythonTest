...
class Dict(dict):
    '''''
    一下是docttest.testmod()会调用作为测试的内容 也就是简单的unittest 单元测试 simple dict but spport access as x.y style 
    >>> d1 = Dict() 
    >>> d1['x'] = 100
    >>> d1.x 
    100
...
def next_id(t=None):
    ...
    if t is None:
        t = time.time()
    return '%015%s000'%(int(t*1000),uuid.uuid.hex)
'''''
  @time 2015-02-01
  @method _profiling记录sql的运行状态
  '''
def _profiling(start,sql=''):
    '''''
    单下划线开头的方法名或属性不会在from moduName import * 中被导入，也就是说只有本模块可以访问
    '''
    t = time.time()-start
    if t >0.1:
...
def create_engine(host,user,password,database,*kw):
#def create_engine(user,password,database,host='localhost',port=3366,**kw):
    #import mysql.connector
    #from sqlalchemy import create_engine
    import MySQLdb
    import sys
    global engine #global关键字 说明这个变量是在外部定义的，这是一个全局变量
    if engine is not None:
        raise DBError('Engin is already initialized')
        #如果连接已经存在表示连接重复，则抛s出一个数据库异常
    params = (host,user,password,database)
#    params = dict(user=user,password=password,database=database,host=host,port=port)
        #（参数）保存数据库的链接信息
#    defaults=dict(use_unicode=True,charset='utf8',collation='utf8_genneral_ci',autocommit=False)#保存链接的设置，编码等
#    for k,v in defaults.iteritems():#将defaults和kw中的键值对保存到params中 如果一个key两边都存在那么保存kw的
#        params[k]=kw.pop(k,v)#pop函数会将key为k的键值对删除并且返回k对应的v 如果k在kew中不存在 name将会返回v
#    params.update(kw)
#    params['buffered'] = True
   # print params
   # conss = MySQLdb.connect(params)
   # print conss
    con = MySQLdb.connect(host,user,password,database);
   # logging.info(con)
    engine = _Engine(lambda:con)
    #C = engine
    #C.close()
    
   # engine = _Engine(lambda:MySQLdb.connect(params))
#       engine = _Engine(lambda:mysql.connector.connect(**params))
       #engine = _Engine(lambda:create_engine(**params))
#在这里lanbda：..返回的是一个函数而不是一个connection对象
        #test connection ...
    logging.info('Init mysql engin <%s> ok '%hex(id(engine)))
        #hex--十六进制，bin--二进制
...
#        self.connection.close()
        if self.connection:
            ...
            #connection.close()
            self.connection = None
'''接下来解决对于不同的线程数据库链接应该是不一样的，于是创建一个变量是threadlocal的对象'''
...
#所以当需要数据库连接的时候就是使用它来创建
_db_ctx = _DbCtx()
'''''=====================================以上通过_db_ctx就可以打开和关闭链接=============================================='''
#通过with语句让数据库链接可以自动创建和关闭
'''''
with语句：
  with后面的语句会返回_ConnectionCtx对象，然后调用这个对象的__enter__方法得到返回值，返回值赋值给as后面的变量然后执行
  with下面的语句 执行完毕后 调用那个对象的 __exit__()方法
  '''
class _ConnectionCtx(object):
    '''''
    _ConnectionCtx object that can open and close connection context._ConnectionCtx object ca
...
class _TransactionCtx(object):
    ...
    def __enter__(self):
        ...
        logging.info('begin transactions....' if _db_ctx.transactions==1 else 'join current trans')
...
#    '''
    create_engine('localhost','testuser1','test1','testdb')
#    '''''
#    a()
#    print _db_ctx
#    '''
...
#    '''
    ...
    u1 = select_one('select * form Write where id = ?',4)
    print u1
    #import doctest
    #doctest.testmod()
